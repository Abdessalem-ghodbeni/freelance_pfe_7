      ...
      (res: any) => {
        ...
        if (res && res.userDetails && res.userDetails.enabled) {
          ...
          if (res.userDetails.role === Role.ADMIN) {
            this.router.navigateByUrl('/admin/list-articles');
          }
          // }
          else if (res.userDetails.role === Role.AGENT) {
            this.router.navigateByUrl('/agent/demande_chequier');
          } else {
            this.router.navigateByUrl('/client/passer-commande');
          }
        }
      },
      (err) => {
